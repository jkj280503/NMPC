...
class NMPCLeadLagNode(Node):
    def __init__(self):
        ...
        self.kv_nominal = 0.94540880
        self.kr_nominal = 0.94517433
        ...
        self.ll_K_lag = 0.8
        self.ll_K_lead = 0.4
        self.ll_Kd_time_v = 0.0
        self.ll_dtime_err_max = 2.0
        self.ll_dv_max = 0.35
        self.time_err_alpha = 0.7
        self.time_err_filt = 0.0
        self.prev_time_err = 0.0
        self.time_err_initialized = False
        self.ll_decay_rate = 0.5
        self.enable_curve_speed_limit = True
        self.ll_ay_max = 1.2     
        self.ll_kappa_eps = 1e-3
        self.use_slope_feedforward = True
        self.ff_slope_gain = 0.3

        self.ll_terminal_window = 1.0
        self.ll_terminal_lag_thr = -0.03
        self.ll_K_lag_terminal = 1.2
        self.ll_dv_max_terminal = 0.45
        self.ll_terminal_min_acc_ff = 0.0
    ...
    def control_loop(self):
        ...
        time_err_raw = e_t / max(abs(v_ref_now), self.ll_v_eps)
        time_err_raw = float(np.clip(
            time_err_raw,
            -self.ll_time_err_max,
            self.ll_time_err_max
        ))

        if not self.time_err_initialized:
            self.time_err_filt = time_err_raw
            self.prev_time_err = time_err_raw
            self.time_err_initialized = True
        else:
            self.time_err_filt = (
                self.time_err_alpha * self.time_err_filt
                + (1.0 - self.time_err_alpha) * time_err_raw
            )

        time_err = float(self.time_err_filt)

        d_time_err = (time_err - self.prev_time_err) / self.dt
        d_time_err = float(np.clip(
            d_time_err,
            -self.ll_dtime_err_max,
            self.ll_dtime_err_max
        ))
        self.prev_time_err = time_err

        time_to_go = self.t_final - t_ref_now
        near_terminal = time_to_go < self.ll_terminal_window
        terminal_lagging = near_terminal and (time_err < self.ll_terminal_lag_thr)

        if time_err < 0.0:
            if terminal_lagging:
                K_time = self.ll_K_lag_terminal
                dv_limit = self.ll_dv_max_terminal
            else:
                K_time = self.ll_K_lag
                dv_limit = self.ll_dv_max
        else:
            K_time = self.ll_K_lead
            dv_limit = self.ll_dv_max

        dv_lag_base = -K_time * time_err - self.ll_Kd_time_v * d_time_err
        dv_lag_base = float(np.clip(dv_lag_base, -dv_limit, dv_limit))
        ...
        for i in range(self.Np + 1):
            ...
            if terminal_lagging:
                ...
                decay_rate = self.ll_decay_rate
            
            ll_decay = math.exp(-decay_rate * i * self.dt)

            dv_lag_i = dv_lag_base * ll_decay

            v_ref_ll = v_ref + dv_lag_i

            if abs(v_ref) < self.ll_v_eps:
                kappa_ref = 0.0
                v_ref_ll = float(np.clip(v_ref_ll, self.v_min, self.v_max))
                omega_ref_ll = float(np.clip(omega_ref, self.omega_min, self.omega_max))
            else:
                kappa_ref = omega_ref / v_ref

                if self.enable_curve_speed_limit and abs(kappa_ref) > self.ll_kappa_eps:
                    v_curve_max = math.sqrt(
                        self.ll_ay_max / max(abs(kappa_ref), self.ll_kappa_eps)
                    )
                    if v_ref_ll > v_ref:
                        v_ref_ll = min(v_ref_ll, v_curve_max)
            ...
            if i < self.Np:
                ...
                if terminal_lagging:
                    ...
                    decay_rate = self.ll_decay_rate
                ll_decay_next = math.exp(-decay_rate * (i + 1) * self.dt)

                dv_lag_next = dv_lag_base * ll_decay_next

                v_next_ll = v_next + dv_lag_next

                if abs(v_next) < self.ll_v_eps:
                    kappa_next = 0.0
                    v_next_ll = float(np.clip(v_next_ll, self.v_min, self.v_max))
                    omega_next_ll = float(np.clip(omega_next, self.omega_min, self.omega_max))
                else:
                    kappa_next = omega_next / v_next

                    if self.enable_curve_speed_limit and abs(kappa_next) > self.ll_kappa_eps:
                        v_curve_max_next = math.sqrt(
                            self.ll_ay_max / max(abs(kappa_next), self.ll_kappa_eps)
                        )
                        if v_next_ll > v_next:
                            v_next_ll = min(v_next_ll, v_curve_max_next)
                ...
                if terminal_lagging:
                    a_ref_ll = max(a_ref_ll, self.ll_terminal_min_acc_ff)
