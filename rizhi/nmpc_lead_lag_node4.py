...
class NMPCLeadLagNode(Node):
    ...
    def path_callback(self, msg):
        ...
        dx_raw = np.diff(x_pts)
        dy_raw = np.diff(y_pts)
        ds_raw = np.sqrt(dx_raw**2 + dy_raw**2)

        keep = np.hstack(([True], ds_raw > 1e-6))
        x_pts = x_pts[keep]
        y_pts = y_pts[keep]
        ...
        theta_pts[:-1] = np.arctan2(np.diff(y_pts), np.diff(x_pts))
        theta_pts[-1] = theta_pts[-2]
        theta_pts = np.unwrap(theta_pts)
        
        sample_idx = np.linspace(0, len(x_pts)-1, min(5, len(x_pts)), dtype=int)
        sig_xy = np.column_stack((x_pts[sample_idx], y_pts[sample_idx])).flatten()
        path_signature = tuple(np.round(sig_xy, 4))
