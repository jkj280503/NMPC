...
######灰箱模型参数，待定
        ...
        self.g = 9.81
        self.road_grade_deg = float(self.declare_parameter('road_grade_deg', 0.0).value)
        self.road_grade_rad = math.radians(self.road_grade_deg)
    ...
    def init_casadi_solver(self):
        ...
        k1 = f_continuous(x, cmd, d_hat, grade)
        k2 = f_continuous(x + self.dt / 2 * k1, cmd, d_hat, grade)
        k3 = f_continuous(x + self.dt / 2 * k2, cmd, d_hat, grade)
        k4 = f_continuous(x + self.dt * k3, cmd, d_hat, grade)

        x_next = x + self.dt / 6 * (k1 + 2 * k2 + 2 * k3 + k4)

        F_discrete = ca.Function(
            'F_discrete',
            [x, cmd, d_hat, grade],
            [x_next]
        )
...
####从轨迹中得到的时间关联参数
    ...
    def get_grade_preview(self, t_query: float) -> float:
        return float(self.road_grade_rad)
...
###确定按照时间计划在的位置角度
        ...
        grade0 = self.get_grade_preview(t_ref_now)
        ...
        for i in range(self.Np + 1):
            ...
            if i < self.Np:
                Grade_ref_mod[0, i] = self.get_grade_preview(t_i)
