...
####-------------------------------####终点的参考速度设定
        ...
        self.goal_pos_tol = 0.10       # m
        self.goal_v_tol = 0.05         # m/s
        self.goal_w_tol = 0.05         # rad/s
        self.enter_goal_time = None
        self.stop_goal_time = None
        self.stop_condition_start_time = None
        self.stop_hold_required = 0.20 
    ...
    def control_loop(self):
        ...
        # ========== 辅助指标：首次进入终点邻域 ==========
        # 这个只说明车辆什么时候进入终点附近，不再作为主要到达时间误差
        if self.enter_goal_time is None and goal_dist < self.goal_pos_tol:
            self.enter_goal_time = elapsed
            self.get_logger().info(
                f"首次进入终点邻域: "
                f"t_enter={self.enter_goal_time:.3f}s, "
                f"t_ref_final={self.t_final:.3f}s, "
                f"仅作为辅助指标"
            )

        # ========== 主指标：终端状态达成时间 ==========
        # 实际车辆必须同时满足：到达终点附近 + 速度接近终端参考速度 + 角速度接近终端参考角速度
        terminal_condition_ok = (
            goal_dist < self.goal_pos_tol
            and abs(self.v_act - self.terminal_v_ref) < self.goal_v_tol
            and abs(self.omega_act - self.terminal_omega_ref) < self.goal_w_tol
        )

        if self.stop_goal_time is None:
            if terminal_condition_ok:
                if self.stop_condition_start_time is None:
                    self.stop_condition_start_time = elapsed

                # 连续满足 stop_hold_required 秒，才认为真正完成终端状态
                if elapsed - self.stop_condition_start_time >= self.stop_hold_required:
                    self.stop_goal_time = elapsed
                    stop_error = self.stop_goal_time - self.t_final

                    self.get_logger().info(
                        f"终端状态达成: "
                        f"t_stop={self.stop_goal_time:.3f}s, "
                        f"t_ref_final={self.t_final:.3f}s, "
                        f"误差={stop_error:.3f}s"
                    )
            else:
                self.stop_condition_start_time = None

        # ========== 控制停止逻辑 ==========
        # 参考时间已经到达终点，并且实际车辆进入终点附近，就发送停车命令
        if t_ref_now >= self.t_final and goal_dist < self.goal_pos_tol:
            self.stop_robot()
            self.get_logger().info("已到达终点区域，发送停车命令！", once=True)
            return
