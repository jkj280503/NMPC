...
class NMPCLeadLagNode(Node):
    ...
    def find_projected_time(self, t_center, window=1.0):
        if self.t_track is None or len(self.t_track) < 2:
            return float(t_center)

        t_min = max(0.0, float(t_center) - window)
        t_max = min(self.t_final, float(t_center) + window)

        mask = (self.t_track >= t_min) & (self.t_track <= t_max)

        if np.any(mask):
            ts = self.t_track[mask]
        else:
            ts = self.t_track

        xs = self.traj_interp['x_of_t'](ts)
        ys = self.traj_interp['y_of_t'](ts)

        d2 = (xs - self.x_act) ** 2 + (ys - self.y_act) ** 2
    ...
    def control_loop(self):
        ...
        t_sched = float(np.clip(self.t_ref_offset + elapsed, 0.0, self.t_final))
        t_proj = self.find_projected_time(t_sched, window=1.0)
        t_ref_now = t_sched
        t_geo_now = t_proj

        x_ref_k = float(self.traj_interp['x_of_t'](t_geo_now))
        y_ref_k = float(self.traj_interp['y_of_t'](t_geo_now))
        theta_ref_k = float(self.traj_interp['theta_of_t'](t_geo_now))
        ...
        for i in range(self.Np + 1):
            t_i = min(t_proj + i * self.dt, self.t_final)
            ...
            if i < self.Np:
                # ========== next-step reference for acceleration feedforward ==========
        ...
        alpha_now = float(self.traj_interp['alpha_of_t'](t_proj))
        beta_now = float(self.traj_interp['beta_of_t'](t_proj))
        mu_now = float(self.traj_interp['mu_of_t'](t_proj))
        self.get_logger().info(
            f"Δp_geo: {delta_p:.3f} m | "
            f"t_sched: {t_sched:.2f} | t_proj: {t_proj:.2f} | "
            f"time_err: {time_err:.3f} s | dv_lag: {dv_lag_base:.3f} | "
