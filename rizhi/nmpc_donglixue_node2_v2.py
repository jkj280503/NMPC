...
class NMPCLeadLagNode(Node):
    def __init__(self):
        ...
        self.use_measured_vy = False
        self.vy_lpf_alpha = 0.15
        self.vy_meas_filt = 0.0

######灰箱模型参数，待定
        self.declare_parameter('tau_u', 0.25)
        self.declare_parameter('tau_r', 0.18)
        self.declare_parameter('k_vr', 0.55)
        self.declare_parameter('c_v', 1.20)
        self.declare_parameter('c_r', 0.35)

        self.tau_u = float(self.get_parameter('tau_u').value)
        self.tau_r = float(self.get_parameter('tau_r').value)
        self.k_vr = float(self.get_parameter('k_vr').value)
        self.c_v = float(self.get_parameter('c_v').value)
        self.c_r = float(self.get_parameter('c_r').value)

        self.declare_parameter('enable_id_log', True)
        self.declare_parameter('id_log_path', '~/nmpc_id_log.csv')

        self.enable_id_log = bool(self.get_parameter('enable_id_log').value)
        self.id_log_path = os.path.expanduser(str(self.get_parameter('id_log_path').value))
        self._id_log_header_written = False

        # ===== 辨识实验模式参数 =====
        self.declare_parameter('id_mode', False)       # True: 不跑NMPC，只做阶跃实验
        self.declare_parameter('id_pre_time', 2.0)     # 阶跃前保持时间
        self.declare_parameter('id_hold_time', 4.0)    # 阶跃保持时间
        self.declare_parameter('id_post_time', 3.0)    # 回零/回落后保持时间

        self.declare_parameter('id_u_pre', 0.0)
        self.declare_parameter('id_u_step', 0.0)
        self.declare_parameter('id_u_post', 0.0)

        self.declare_parameter('id_r_pre', 0.0)
        self.declare_parameter('id_r_step', 0.0)
        self.declare_parameter('id_r_post', 0.0)

        self.id_mode = bool(self.get_parameter('id_mode').value)
        self.id_pre_time = float(self.get_parameter('id_pre_time').value)
        self.id_hold_time = float(self.get_parameter('id_hold_time').value)
        self.id_post_time = float(self.get_parameter('id_post_time').value)

        self.id_u_pre = float(self.get_parameter('id_u_pre').value)
        self.id_u_step = float(self.get_parameter('id_u_step').value)
        self.id_u_post = float(self.get_parameter('id_u_post').value)

        self.id_r_pre = float(self.get_parameter('id_r_pre').value)
        self.id_r_step = float(self.get_parameter('id_r_step').value)
        self.id_r_post = float(self.get_parameter('id_r_post').value)

        self.id_start_time = None
        self.id_finished = False
    ...
    def log_id_data(self, t_now, u_cmd, u_meas, r_cmd, r_meas):
        if not self.enable_id_log:
            return

        need_header = (not os.path.exists(self.id_log_path)) or (os.path.getsize(self.id_log_path) == 0)

        with open(self.id_log_path, 'a', newline='') as f:
            writer = csv.writer(f)

            if need_header:
                writer.writerow(['t', 'u_cmd', 'u_meas', 'r_cmd', 'r_meas'])
    ...
    def publish_direct_cmd(self, u_cmd, r_cmd, t_now):
        u_cmd = float(np.clip(u_cmd, self.u_cmd_min, self.u_cmd_max))
        r_cmd = float(np.clip(r_cmd, self.r_cmd_min, self.r_cmd_max))
        ...
        # 记录辨识数据
        self.log_id_data(t_now, u_cmd, self.u_body_act, r_cmd, self.r_act)

        # 也顺便把调试量发出去，方便 rqt_plot 看
    ...
    def run_id_experiment(self):
        ...
        if self.id_start_time is None:
            self.id_start_time = now
            self.get_logger().info(
                f'开始辨识实验 | '
                f'u: {self.id_u_pre:.2f} -> {self.id_u_step:.2f} -> {self.id_u_post:.2f} | '
                f'r: {self.id_r_pre:.2f} -> {self.id_r_step:.2f} -> {self.id_r_post:.2f}'
            )

        elapsed = (now - self.id_start_time).nanoseconds * 1e-9

        if elapsed < self.id_pre_time:
            u_cmd = self.id_u_pre
            r_cmd = self.id_r_pre
        elif elapsed < self.id_pre_time + self.id_hold_time:
            u_cmd = self.id_u_step
            r_cmd = self.id_r_step
        elif elapsed < self.id_pre_time + self.id_hold_time + self.id_post_time:
            u_cmd = self.id_u_post
            r_cmd = self.id_r_post
        else:
            self.stop_robot()
            self.id_finished = True
            self.get_logger().info('辨识实验结束，已自动停车。')
            return

        self.publish_direct_cmd(u_cmd, r_cmd, elapsed)
...
###确定按照时间计划在的位置角度
        ...
        self.log_id_data(elapsed, u_cmd, self.u_body_act, r_cmd, self.r_act)
