...
class NMPCLeadLagNode(Node):
    def __init__(self):
        ...
        # 纵坡项参数。
        # slope_gain = 1.0 表示完全按重力沿坡面分量补偿；
        # 如果 Gazebo 中速度控制器很强、坡度对速度影响较弱，可先调小到 0.3~0.7。
        self.g_const = 9.81
        self.slope_gain = 0.2
        self.max_slope_rad = 0.45        # 纵坡角限幅，约 ±25.8 deg，防止高程噪声导致求解异常
        ...
        self.get_logger().info("初始化 CasADi NMPC 求解器：5状态 + 纵坡动力学版本...")
    ...
    def init_casadi_solver(self):
        self.Np = 20
        self.nx = 5       # 版本2：X, Y, theta, vx, r
        self.nu = 2       # v_cmd, omega_cmd
        self.np_terrain = 1  # alpha：路径切向纵坡角
    ...
    def path_callback(self, msg):
        ...
        x_pts, y_pts, z_pts, theta_pts, t_pts = [], [], [], [], []
        t0_sec = None
        ...
        self.get_logger().info(
            f"纵坡预瞄已建立: alpha_min={np.min(alpha_pts):.4f} rad, "
